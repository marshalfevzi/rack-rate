...
Frozen counts: 178 pairs, 28 best routes, 5 known gaps, cross-check median 1.601
/ min 1.006 / max 6.713 over 11 pairs.
...
3. `days_for_full_run` capped by the rolling window (5 h), not only the monthly
   quota;
4. `known_gaps` carried through as a backlog, not dropped.

The JSON is data, not prose: the port compares `pairs` (178), `best_routes` (28),
`cross_check.pairs` (11) and `cross_check.summary.{median_ratio,min_ratio,max_ratio,pair_count}`.
