...
2. Read each reference document reported by `pm_doc_check` with the `doc-append-record` code
   and relocate every per-task or per-stage record section. Measurements and build notes go into
   the owning `docs/pm/<M>/done/<ID>.md` `## Session` section when that task exists, and
   milestone- or stage-level material goes to the flat `docs/archive/<M>-<slug>.md`. Keep one
   reconciled section in the reference document describing current state. Preserve content; never
   delete a record that has no destination.
