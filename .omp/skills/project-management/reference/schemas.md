...
The source documents under `docs/pm/` own the plan. Their YAML frontmatter is parsed into a model; `docs/pm/plan.yml` is generated from that model. Dates use ISO `YYYY-MM-DD` strings. A nullable field is written as YAML `null` when it has no value. Lists are YAML lists, including an empty list.
...
| `project.commands.format`      | string or null          |      yes | Formatter command that `pm_task_finish` runs before committing. |
| `project.commands.gate`        | string or null          |      yes | Project quality gate command.                                   |
...
The repository's initial config declares `COR`, `DATA`, `APP`, `UI`, `DOC`, and `PM` prefixes; `UI` is the only one with `impeccable: true`.
...
Destinations: `docs/pm/<M>/todo/<ID>.md`, `docs/pm/<M>/done/<ID>.md`, or `docs/pm/unplanned/<ID>.md`.
...
A task in `done/` has `status: done`, a completion date, and a `## Session` section appended by `pm_task_finish`. A task in `todo/` or `unplanned/` has a non-done status.
...
Destination: `docs/archive/<M>-<slug>.md`. The archive is a flat file and retains the milestone record, task records in order, and retro bodies.
...
The first line is exactly `# GENERATED by pm_plan_sync — do not edit`. The YAML body has this fixed key order and nesting:
...
The generated plan is a pure function of the source documents. Top-level keys are `project`, `milestones`, and `archived`, in that order. `project` keys are `name`, `prd`, `product`, `design`, and `architecture`, in that order. Each milestone uses `id`, `title`, `status`, `file`, `tasks`, and `retro`; each task uses `id`, `title`, `description`, `status`, `pre`, `file`, and `impeccable`; each archive uses `id`, `title`, `status`, `file`, in those orders. Lists use block style except empty lists, which render as `[]`.

Strings are double-quoted only when they contain `:`, `#`, leading or trailing whitespace, or start with `[`, `{`, `-`, or `?`; otherwise they are emitted bare. `null` renders as `null`.
