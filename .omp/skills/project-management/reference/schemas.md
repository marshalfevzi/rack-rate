...
The generated plan is a pure function of the source documents. Top-level keys are `project`,
`milestones`, and `archived`, in that order. `project` keys are `name`, `prd`, `product`, `design`,
and `architecture`, in that order. Each milestone uses `id`, `title`, `status`, `file`, `tasks`, and
`retro`; each task uses `id`, `title`, `description`, `status`, `pre`, `file`, and `impeccable`; each
archive uses `id`, `title`, `status`, `file`, in those orders. Lists use block style except empty
lists, which render as `[]`.
