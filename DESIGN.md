...
The persisted preference namespace is `rack-rate:prefs:v1`, planned by `docs/pm/M6/todo/APP-601.md`: `apps/site/src/lib/prefs.ts` does not exist in this tree yet, so no mark is persistable today. When that store lands it carries `ignoredModels`, `ignoredPlans`, `paidPlans`, `vendor`, `currency`, `budgetCeiling`, `weights`, and `benchmarkFilters`; until then every mark above is session or URL state. Marks that describe evidence (`!`, `?`) stay with data and confidence; marks that describe a transient interaction (`·`, `+`) stay in URL/session state. This preserves the planned preference schema without inventing a key.
