...
Edit `data/plans.json`, then run `bun run data:build` (which runs `bun run validate` then `bun run compute`) and `bun run data:check`. The committed `data/derived.json` is regenerated by `data:build`; `data:check` recomputes it in memory and exits 1 if the committed output is stale. CI runs `data:check`, so it must stay green.
