...
Edit `data/plans.json`, then run `bun run validate` and `bun run compute`; Stage 2 owns both commands (they are not implemented yet—the CLI is currently a stub that exits 1). The derived output is regenerated by `bun run compute` and is not hand-committed in this stage.
...
`bun run fetch:deepswe` is the planned refresh path (Stage 2.2); its planned `--diff` mode will print what moved on the leaderboard without writing anything. If the endpoint list in that command has gone stale, fixing it is a welcome PR on its own.
