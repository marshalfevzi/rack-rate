...
description: "5.11 /explore — the instrument panel. Files: apps/site/src/pages/explore.astro. Work: keep all six chart plates (Pareto, bump/rank, heatmap, slope, waterfall, radar) in the new grammar, each with its own control row, its reserved height,..."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.11 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
