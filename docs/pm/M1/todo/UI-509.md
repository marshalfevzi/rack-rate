...
description: "5.9 Chart grammar — re-theme the shared frame and all six builders. Files: apps/site/src/lib/charts/{theme,frame,registry,mount}.ts, the six option builders (pareto, bump, heatmap, slope, waterfall, radar), their *-payload.ts and *-page...."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.9 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
