...
description: "5.3 Shell — status band, lane rail, drawer, workspace, footer index. Files: apps/site/src/layouts/Base.astro, apps/site/src/layouts/Page.astro, new apps/site/src/components/StatusBand.astro, apps/site/src/components/LaneRail.astro, apps/..."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.3 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
