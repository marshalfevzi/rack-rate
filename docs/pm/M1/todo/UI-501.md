...
description: "5.1 Token foundation and scheme inversion. Files: apps/site/src/styles/global.css. Work: replace the nine incumbent tokens (canvas, panel, rule, ink, dim, adjusted, measured, api, api-ink) with the frozen ten, adding panel-2, rule-strong..."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.1 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
