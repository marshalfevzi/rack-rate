...
description: "5.6 / — the split console. Files: apps/site/src/pages/index.astro. Work: recompose the entry as the split console: the left selection column is plan → models → tasks per month, pre-filled with a real committed plan and its measured-again..."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.6 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
