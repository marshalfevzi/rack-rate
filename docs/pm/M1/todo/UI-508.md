...
description: "5.8 /plans and /plans/[slug]. Files: apps/site/src/pages/plans/index.astro, apps/site/src/pages/plans/[slug].astro. Work: the index becomes the sortable plan listing — rank, plan, provider, price/month, quota model, key quota, rolling wi..."
status: todo
milestone: M1
pre: []
kind: design
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 5 task 5.8 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M1/README.md`. The task has not been enriched or rewritten.
