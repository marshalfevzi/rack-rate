...
`bun install` succeeds; `bun run check` runs and passes (typechecking
only the migrated schema for now); `bun test` reports zero tests without
error. _(Install verified during initialization: 301 packages, `bun.lock`
