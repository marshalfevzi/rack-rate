...
description: "7.2b Artificial Analysis publication decision, made explicitly here rather than by default. Three defensible states, in ascending exposure; record which one ships and why: 1. AA_PUBLISH=0 (default, current). No AA values anywhere; the So..."
status: todo
milestone: M3
pre: []
kind: docs
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 7 task 7.2b by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M3/README.md`. The task has not been enriched or rewritten.
