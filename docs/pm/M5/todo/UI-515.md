...
**Re-derive the design record from the built world.**
  ...
  **Work:** re-derive each document from the shipped code rather than editing it
  ...
  `ARCHITECTURE.md` its Console Listing section — both schemes with their
  ...
  **Done when** every value, token, metric and docking in `DESIGN.md`,
  `docs/design/surfaces.md`, the surface brief and `ARCHITECTURE.md` is one
