...
description: "6.1 src/lib/prefs.ts — typed nanostores persistent stores under one versioned key namespace, rack-rate:prefs:v1, holding { ignoredModels, ignoredPlans, paidPlans, vendor, currency, budgetCeiling, weights, benchmarkFilters }. SSR-safe (no..."
status: todo
milestone: M2
pre: []
kind: feature
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 6 task 6.1 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M2/README.md`. The task has not been enriched or rewritten.
