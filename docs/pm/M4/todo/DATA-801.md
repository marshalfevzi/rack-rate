...
description: "8.1 Refresh the eight plan rows that fail the anchor check. Files: packages/data-cli/src/commands/fetch-plans.ts, data/plans.json, data/sources.json, ARCHITECTURE.md. Why: data/plans.json is still at its Stage 1 revision. Measured 2..."
status: todo
milestone: M4
pre: []
kind: fix
...
Migrated verbatim from the pre-PM `PLAN.md` (deleted after the migration; recover with `git show d95e6ef:PLAN.md`) stage 8 task 8.1 by the PM init on 2026-09-17; the stage's boundaries and acceptance live in `docs/pm/M4/README.md`. The task has not been enriched or rewritten.
