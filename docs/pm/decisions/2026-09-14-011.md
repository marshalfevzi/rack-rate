...
pure halves. Builders stay pure — `(data) => ChartOption`, no `use()`, no DOM,
no clock — and `mount.ts` is the only module in the directory that touches
`window`.

## Decision

The sentence the 4.1 entry originally carried — that an option object therefore
cannot carry a series type no stage registered, and a missing `use()` is a type
error rather than a blank chart — is false and is superseded by "Second review
finding" and "Third review finding"; the option type accepts any extra key, and
the registry checks series names at mount time instead.
...
- A missing `use()` is caught at mount time, not by the option type: the
  `SERIES_INSTALLS` guard rejects an option whose declared series family has no
  registered row, and the refused mount disposes its instance so a retry reports
  the real problem.
- The 2026-09-17 plan review added the obligation to unit-test the five builders
  that were only ever verified by rendering. That obligation is `UI-509` in
  `docs/pm/M5/` (`docs/pm/M5/README.md`, `docs/pm/M5/todo/UI-509.md`).
